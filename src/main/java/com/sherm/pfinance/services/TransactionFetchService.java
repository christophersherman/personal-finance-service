...
public class TransactionFetchService {
  ...
  public void fetchAndSaveTransactions() {
    ...
    for (Transactions t : lastDaysTransactions) {
      // revisit how to do this... getTransactionbyId has a .orElse null return.
      // probs should build a exists function
  ...
  public List<Transactions> fetchUpBankTransactions() {
    ...
    for (JsonNode transactionNode : jsonResponse) {
      ...
      String description =
          transactionNode.get("attributes").get("description").asText();
      ...
      String accountId =
          relationships.get("account").get("data").get("id").asText();
      // get currencyType
      CurrencyType currType =
          CurrencyType.valueOf(amount.get("currencyCode").asText());
      ...
      transaction.setDate(LocalDate.parse(
          transactionNode.get("attributes").get("settledAt").asText(),
  ...
  public String getUpBankTimeISO() {
    // This function returns the string value of 24hours prior to now, in
    // ISO8601 format
    ZonedDateTime now = ZonedDateTime.now();
    ZonedDateTime twentyFourHoursAgo = now.minusHours(24);
    return URLEncoder.encode(
        twentyFourHoursAgo.format(
